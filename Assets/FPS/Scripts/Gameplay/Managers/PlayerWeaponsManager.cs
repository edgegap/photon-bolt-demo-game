...
namespace Unity.FPS.Gameplay
{
    ...
    public class PlayerWeaponsManager : MonoBehaviour
    {
        ...
        [SerializeField] GameObject weaponFirstPersonParent;
        [SerializeField] GameObject weaponThirdPersonParent;
        ...
        void Start()
        {
            ...
            if (!m_PlayerCharacterController.IsLocalPlayer)
            {
                // Put weapon in 3rd person view
                Vector3 weaponGlobalPosition = weaponSocket.transform.position;
                weaponSocketRotation = weaponSocket.transform.rotation;
                weaponSocket.transform.parent = weaponThirdPersonParent.transform;
                weaponSocket.transform.position = weaponGlobalPosition;
                weaponSocket.transform.localPosition = Vector3.zero;

                // Give default layer to weapon so it is properly hidden by obstacles
                Transform[] weaponChildren = weaponSocket.GetComponentsInChildren<Transform>();
        ...
        void Update()
        {
            if (m_PlayerCharacterController == null || !m_PlayerCharacterController.IsLocalPlayer) return;
        ...
        void LateUpdate()
        {
            if (!m_PlayerCharacterController.IsLocalPlayer)
            {
                if (weaponSocketRotation == null) weaponSocketRotation = weaponSocket.transform.rotation;
                weaponSocket.transform.rotation = weaponSocketRotation;
            }
