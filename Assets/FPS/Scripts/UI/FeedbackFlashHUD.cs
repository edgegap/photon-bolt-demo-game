...
namespace Unity.FPS.UI
{
    public class FeedbackFlashHUD : MonoBehaviour
    {
        ...
        void Awake() => PlayerEvents.OnLocalPlayerReady += SubscribeToPlayerDamageEvents;
        ...
        void SubscribeToPlayerDamageEvents(GameObject player)
        {
            if (!player.GetComponent<PlayerCharacterController>().IsLocalPlayer) return;

            // Subscribe to player damage events
            if (PlayerCharacterController.LocalPlayer != null)
            {
